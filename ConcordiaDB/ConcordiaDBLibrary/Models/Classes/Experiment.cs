...
public class Experiment : TrelloEntity
{
    ...
    public static bool FromBitToBool(byte value)
    {
        return !(value == 0);
    }
    ...
    public override string ToString()
    {
        return string.Concat($"{base.ToString()}, ",
               $"{nameof(Experiment.Name)}:{Name}, ",
               $"{nameof(Experiment.Description)}:{Description}, ",
               $"{nameof(Experiment.Loaded)}:{Loaded}, ",
               $"{nameof(Experiment.StartDate)}:{StartDate}, ",
               $"{nameof(Experiment.DueDate)}:{DueDate}, ",
               $"{nameof(Experiment.PriorityId)}:{PriorityId}, ",
               $"{nameof(Experiment.StateId)}:{StateId}");
    }
}